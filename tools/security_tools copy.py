    ...
    async def run_subfinder(self, domain: str, timeout: int = 120) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"subfinder scan for {domain} timed out after {timeout} seconds.")
                try:
                    process.kill()
                    stdout, stderr = await process.communicate()
                    output = stdout.decode().splitlines()
                    subdomains = [line.strip() for line in output if line.strip()]
                    logger.warning(f"Partial subfinder results for {domain} due to timeout. Found {len(subdomains)} subdomains.")
                    return {
                        "success": True,
                        "tool": "subfinder",
                        "domain": domain,
                    ...
                    logger.error(f"Failed to retrieve partial subfinder results after timeout: {e}")
                    return {"error": f"subfinder scan for {domain} timed out and no results could be retrieved", "success": False}

            if process.returncode == 0:
                output = stdout.decode().splitlines()
                subdomains = [line.strip() for line in output if line.strip()]
                logger.info(f"Successfully processed subfinder results for {domain}. Found {len(subdomains)} subdomains.")
                return {
                    "success": True,
                    "tool": "subfinder",
                    "domain": domain,
                ...
                error_message = stderr.decode().strip()
                logger.error(f"subfinder failed for domain {domain}. Error: {error_message}")
                return {
                    "error": f"subfinder failed: {error_message}",
                    "success": False
                }
        except Exception as e:
            logger.exception(f"Unhandled exception while trying to run subfinder for {domain}")
            return {
                "error": f"Failed to run subfinder: {str(e)}",
                "success": False
            }
